...
class AVLSystem:
    ...
    def create_vehicle_position(self, gps_data):
        ...
        # Use device_id as vehicle label, with fallback
        entity.id = str(gps_data.get('device_id', '125'))

        # Create VehiclePosition
        vehicle = entity.vehicle
        
        # Set trip information with proper type conversion
        vehicle.trip.trip_id = str(gps_data.get('trip_id', '4001'))
        vehicle.trip.route_id = str(gps_data.get('route_id', '32'))
        vehicle.trip.direction_id = int(gps_data.get('direction_id', 16))
        
        # Set start date
        current_time = datetime.now()
        vehicle.trip.start_date = current_time.strftime("%Y%m%d")
        
        # Set position information with proper type conversion
        try:
        ...
        # Set vehicle information with proper type conversion
        vehicle.vehicle.id = str(gps_data.get('device_id', '2068'))
        vehicle.vehicle.label = str(gps_data.get('device_id', '130'))
    ...
    def create_trip_update(self, gps_data, gtfs_data):
        ...
        # Get trip ID from GPS data or generate new one
        trip_id = str(gps_data.get('trip_id', '4001'))
        ...
        trip_update.trip.route_id = str(gps_data.get('route_id', '32'))
        trip_update.trip.direction_id = int(gps_data.get('direction_id', 16))
        ...
        # Set vehicle information with proper type conversion
        trip_update.vehicle.id = str(gps_data.get('device_id', '2068'))
        trip_update.vehicle.label = str(gps_data.get('device_id', '130'))
    ...
    def create_service_alert(self, alert_data):
        ...
        time_range.end = int(time.time()) + 3600  # Alert valid for 1 hour
        ...
        alert.cause = 1  # UNKNOWN_CAUSE
        alert.effect = 8  # UNKNOWN_EFFECT
    ...
    def process_data(self):
        ...
        try:
            ...
            # Get vehicle and trip information from config
            vehicle_number = gps_data.get('device_id', '125')
            vehicle_info = self.get_vehicle_info(vehicle_number)
            route_id = gps_data.get('route_id', '55')
            trip_info = self.get_trip_info(route_id)

            # Update GPS data with configuration
            gps_data.update({
                'device_id': vehicle_info['device_id'],
                'trip_id': trip_info.get('trip_id', '351378'),
                'route_id': route_id,
                'direction_id': trip_info.get('direction_id', 16)
            })
